...
class SafetyLayer:
    ...
    ! NEW: The safety layer is made more proactive.
    ...
    def apply(self, action, state):
        ...
        # ! NEW: Proactive safety check. If glucose is approaching hypo and dropping, block insulin.
        # Check if glucose is within the warning zone and falling quickly
        if glucose < self.predictive_low_threshold and rate_of_change < -1.5:
            return np.array([0.0])
        ...
        # ! REVISED: More aggressive scaling for moderate lows
        if glucose < 130 and rate_of_change < -2.0:
            reduced_action = 0.2 * action
