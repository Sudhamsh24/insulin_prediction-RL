...
# --- Local Imports ---
from agents.ddpg_agent import DDPGAgent # ! Using updated DDPG agent
from utils.replay_buffer import ReplayBuffer
from utils.state_management2 import StateRewardManager # ! Using updated state/reward manager
...
def main():
    ...
    # ! Added exploration noise and decay
    ...
    for patient_name in adult_patients:
        ...
        agent = DDPGAgent(env, state_dim, action_dim, n_latent_var, lr, gamma_val, tau) 
        ...
        for i_episode in range(1, max_episodes + 1):
            ...
            for t in range(max_timesteps_per_episode):
                if total_timesteps_taken < learning_starts:
                    ...
                    action = agent.select_action(current_state, noise=exploration_noise)
            ...
            exploration_noise = max(noise_min, exploration_noise * noise_decay_rate)
        ...
        eval_agent = DDPGAgent(eval_env, state_dim, action_dim, n_latent_var, lr, gamma_val, tau)
        ...
        for t in range(max_timesteps_per_episode):
            with torch.no_grad():
                action = eval_agent.select_action(current_state, noise=0.0)
            ...
            current_sim_time = eval_env.unwrapped.env.env.time
            upcoming_carbs = episode_scenario.get_action(current_sim_time).CHO if episode_scenario else 0
            next_unnormalized_state = manager.get_full_state(obs_array[0], upcoming_carbs)
            current_state = manager.get_normalized_state(next_unnormalized_state)
        ...
        plt.plot(time_axis_minutes, glucose_history, label='DDPG Agent Glucose')
...
def main():
    ...
    for patient_name in adult_patients:
        ...
        # ! Pass the device to the agent
        agent = DDPGAgent(env, state_dim, action_dim, n_latent_var, lr, gamma_val, tau, device) 
        manager = StateRewardManager(state_dim)
        safety_layer = SafetyLayer()
        replay_buffer = ReplayBuffer(replay_buffer_size)
        ...
        print(f"--- Training Finished for {patient_name} ---")
        # Save model to CPU for portability
        torch.save(agent.actor.cpu().state_dict(), actor_path)
        print(f"Saved trained model to {actor_path}")
        ...
        # ! Pass the device to the agent for evaluation
        eval_agent = DDPGAgent(eval_env, state_dim, action_dim, n_latent_var, lr, gamma_val, tau, device)
        eval_agent.actor.load_state_dict(torch.load(actor_path))
        eval_agent.actor.eval()
